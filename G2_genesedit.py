...
for file in gff_sorted:
    ...
    gcmd.write(f"sed -n -e 's/^.*ID=\\([^;]*\\).*/\\1./p' {outputdir+file} > {outputdir+basename+'.txt'}")
    gcmd.close()
...
print('\n\tExtracted gene ID lists from '+str(count)+' gff candidate files')
...
for file in a_sorted:
    ...
    for tline in query:
        ...
        for testline in test:
            data = testline.split('\t')
            if line in data[0]:
                outfile.write(data[0]+'\t'+data[1]+'\t'+data[2]+'\t'+data[3])
    query.close()
    outfile.close()
    count +=1
